...
c               with respect to x for a range of values of the
c               order m and a specified number of values for the
c               degree l = m, m+1,... for specified values of the
c               size parameter c and shape parameter x.
c               To calculate the first kind oblate angular
c               functions and their first derivatives with
c               respect to eta for a range of values of m, l,
c               and angle coordinate eta for a specified value of c.
c
...
c
c  oblfcn can be run in either double precision or quadruple precision
...
c  profcn. Here, the kind parameter knd is set by the statement:
...
c  higher accuracy but will significantly increase the run time.
c
c  An alternative to using module param is to create two versions of
c  oblfcn, one for double precision and one for quadruple precision. To
c  do this, replace knd throughout oblfcn with the appropriate value of
...
c     Input Options
C
c  Input parameters are read from unit 1 in the file oblfcn.dat
c  assumed to be located in the directory of the fortran file.
c  Oblfcn.dat contains the following lines of data. (Note that a
c  subroutine version of oblfcn is available where the input
c  parameters and output function values are passed through the
c  call statement. See the website listed above to obtain a free
c  copy).
c
c       line 1:
...
c                   if lnum is less than 2*c/pi it should
c                   be an even integer. If lnum is chosen to
c                   be odd, oblfcn will increase lnum by one.
c
c       line 2:
c          ioprad : (integer)
...
c          x      : value of the radial coordinate x [real(knd)]
c                   (any value can be entered if ioprad = 0)
c
c       line 4:
c          ioparg : (integer)
...
c                   or dimensionless if equal to eta) for which angular
...
c  Note that the specs for the open statements are default. It may
c  be desirable to chose other values, especially for the blocksize,
c  depending on what its default is on your computer.
c
c   fort.20
c
...
c                        statement 1440 in subroutine main)
...
c                        of first kind (f17.14; see statements 1440 and
c                        1460 in subroutine main)
c               is1e   : exponent of the oblate angular function of
c                        first kind (i5; see statements 1440 and 1460 in
c                        subroutine main)
c
...
c                        see statements 1450 and 1470 in subroutine
c                        main)
...
c                        to zero. (i2; see statements 1440, 1450, 1460,
c                        and 1470 in subroutine main). The calculated
c                        angular functions tend to be less accurate the
...
c                        (i.e., the closer theta is to 90 degrees). For
c                        c very large and l - m small, even the angular
c                        functions near eta = 1 can be very inaccurate.
...
c     explanatory. If desired, these files can be suppressed by
c     searching for write(40 and write(50 and commenting out
c     the lines of code found.
c
c   fort.60
c
...
c  with negative index appearing in the joining factor can suffer
...
c
c  Oblfcn was tested using real*8 arithmetic with a precision of 15
c  decimal digits. Testing included values for c equal to 0.0001, 0.001,
c  0.01, 0.1, 1, 10, 50, 100, 200, 500, 1000, 2000, 5000 and 10000, for
c  x equal to 0.0001, 0.001, 0.01, 0.1 and 1.0, for orders m from 0 to
c  200 in unit steps and from 210 to 1000 in steps of 10, and for
c  degrees l from m to m - 1 + lnum, where lnum is sufficiently large
c  so that the magnitudes of r1 and r1d are less than 10**(-300). It is
c  unlikely that one wwould need function values for degrees higher than
c  these. However, oblfcn is expected to provide accurate results for
c  much higher values of lnum.
c
...
c
c     Accuracy Estimates For Small x
c
c  Testing for values of x smaller than 0.0001 produced accuracy
c  estimates similar for those discussed above. However, for x less
c  than about 0.1 the accuracy of r2 for l - m even and of r2d for
c  l - m odd can be less than the estimated accuracy when c is large
c  and l is very near but somewhat below the breakpoint. Here as x
c  becomes smaller, r2 becomes progressively smaller in magnitude than
c  r1 for l - m even and r2d becomes progressively smaller than r1d for
c  l - m is odd. The lower accuracy is most pronounced when the near
c  equality of low-order neighboring eigenvalues is used to obtain r2
c  and r2d. (See below for a brief discussion of this method). Here, a
c  good accuracy estimate for r2 for l - m odd and r2d for l - m even
c  is given by the number of leading digits of agreement, called match,
c  between neighboring paired eigenvalues. Match is a good estimate of
c  the accuracy of r2d for l - m even and r2 for l - m odd. Oblfcn
c  does not use match for this estimate but instead uses the Wronskian.
c  Here computed r1 and r1d values for a given l - m are combined with
c  r2 and r2d values obtained using accurate r1 values from l - m + 1
c  if l - m is even or from l - m - 1 is l - m is odd. A conservative
c  estimate of the corresponding accuracy of r2 for l - m even and r2d
c  for l - m odd is obtained by first taking the absolute value of the
c  logarithm of x to the base 10, truncating it to an integer, and then
c  subtracting the result from match. For example, for x = 0.000001 and
c  match = 12 digits, oblfcn reports an accuracy of 12 decimal digits
c  using the Wronskian. An estimate of the accuracy of r2 for l - m even
c  or r2d for l - m odd is 6 digits. For example, comparison of real*8
c  results with real*16 results for c = 2000 show that for a given value
c  of m there were at most one or two values of l where the accuracy of
c  r2 for l - m even was as low as 6 or 7 digits. And this reduced
c  accuracy occurs only when r2 is smaller in magnitude than r1 by about
c  3 orders of magnitude. This gives an effective accuracy of r1 of 9 or
c  10 digits when forming r3 or r4 for applications involving these
c  functions. The same thing occurs for r2d when l - m is odd. As
c  x becomes smaller than 0.000001, the reduced accuracy for a few
c  values of l is balanced by a similar reduction in relative magnitude
c  of the less accurate function values. Thus their contribution to the
c  solutions of physical problems involving these functions is reduced
c  proportionately. Their absolute accuracy decreases but their
c  effective accuracy remains sufficient for most applications.
c
...
c  l - m that are less accurate than the Wronskian estimate. See the
c  discussion above about the accuracy for small values of x.
c
...
c  possible. This can be full precision but often is not due to the
c  asymptotic nature of the Neumann function series.
c
...
c  lim2 can be much larger than 200000 or so when x is near its lower
c  limit of 0.005 where Neumann expansions may be used. Note that the
c  vector enr returned by the subroutine conver contains scaled ratios
c  where the scaling has been chosen to produce a symmetric matrix for
c  computing  eigenvalues. The scaling factors are removed in subroutine
c  dnorm to obtain the desired d coefficient ratios.
c
...
c  returned to main as dmlmf, idmlmfe and dmlf, idmlfe. Calculation of
c  the Flammer normalization suffers subtraction errors for lower values
c  of l-m and large c that increase as c increases. The value for dmlf
c  will have reduced accuracy in this case.
c
        use param
c
...
c  set array dimensions
        nbp=int(2*c/3.1416)
        if(lnum.le.nbp.and.(2*(lnum/2).ne.lnum)) lnum=lnum+1
...
10      maxp=max(maxn,maxp,maxpdr)
     ...
     1      .and.x.le.0.99e0_knd) maxq=max(maxq,250-int(50*x)+maxm+
     2                                 maxm+4)
        maxdr=maxpdr/2+1
        maxd=maxn/2+1
        maxmp=maxm+maxm+5
...
c
     ...
     1             x,ngau,step1,nstep1,step2,nstep2,step3,nstep3,ioparg,
     2             arg1,darg,narg,maxd,maxdr,maxe,maxe2,maxint,maxj,
     3             maxlp,maxm,maxmp,maxn,maxp,maxpdr,maxq,maxt,neta,
     4             jnenmax,jnebmax,xneu,ndec,nex,kindd,kindq)
c
        end
c
c
     ...
     2                   nstep3,ioparg,arg1,darg,narg,maxd,maxdr,maxe,
     3                   maxe2,maxint,maxj,maxlp,maxm,maxmp,maxn,maxp,
     4                   maxpdr,maxq,maxt,neta,jnenmax,jnebmax,xneu,
     5                   ndec,nex,kindd,kindq)
c
...
c               step1  : first step in eta for the Gaussian quadrature
c                        integration
c               nstep1 : number of equal substeps that step1 is divided
c                        into
c               step2  : second step in eta for the Gaussian quadrature
c                        integration
c               nstep2 : number of equal substeps that step2 is divided
c                        into
c               step3  : third and final step in eta for the Gaussian
c                        quadrature integration
c               nstep3 : number of equal substeps that step3 is divided
c                        into 
...
c               maxe   : dimension of both the vector of diagonal
c                        elements and the vector of subdiagonal elements                   
c                        used to obtain accurate estimates of the
c                        eigenvalues for both even and odd l - m
c               maxe2  : dimension of the vector containing all of the
c                        eigenvalue estimates, including both even and
c                        odd l - m (=2*maxe)          
...
c                        (subroutine r2eta) and the variable eta method
c                        for calculating r1 and r1d (subroutine r1eta);
c                        set equal to 30  
c               jnenmax: number of arrays of ratios of Legendre and
c                        Neumann functions stored as eta is varied in
c                        subroutine r2eta; set equal to 3 so that the
c                        previous 3 sets of ratios are available
c                        to use without recalculating them when one of
c                        thes previous values for eta is used again for
c                        a later value of l. If r2eta will not be
c                        called, jnenmax is set equal to unity.
c               jnebmax: number of arrays of ratios of Legendre and
c                        Bessel functions stored as eta is varied in
c                        subroutine r1eta; set equal to the number of
c                        values of eta that are available. Thus the
c                        ratios never need to be recalculated when one
c                        of the previous values used for eta is used
c                        again for a later value of l
c               xneu   : minimum value of x for which either the
c                        Neumann expansion method r2neu0 or the variable
c                        eta method r2eta will be called                           
...
c  real(knd) scalars
     ...
     1            coefmo,darg,dec,dfnorm,dmlf,dmfnorm,dmlmf,dmsnorm,
     2            dmlms,dmlms1,dneg,dc01,eigval,em,etaval,fac1,fac1d,
     3            fac2,fac2d,factor,factor1,pcoefn,pi,qdm0,qdm1,qm0,
     4            qm1,rm,rm2,r1cin,r1cm,r1dcin,r11c,r1dcm,r1d1c,r1ec,
     5            r1dec,r2ic,r2dic,r2lc,r2dlc,r2l1c,r2dl1c,r2nc,r2dnc,
     6            r2ec,r2dec,step1,step2,step3,ten,termpq,t1,t2,t3,t4,
     7            t5,t6,t7,wm,x,xb,xbninp,xl,xneu,wronc,wronca,wroncb,
     8            wront
...
c  real(knd) arrays with dimension maxn
     ...
     1            sneudfe(maxn),sneufsv(jnenmax,maxn),
     2            sneudfsv(jnenmax,maxn)
c
...
c
c  compute the coefficients in the bouwkamp method
...
310             continue
c
c  gamma coefficients (gliste) for l-m even
              j=jlowe
...
340             continue
c
c  gamma coefficient (glist0) for l-m odd
              j=jlowo
...
360           continue
     ...
     1                                 maxd,ioprad,kindd,kindq,eigval,
     2                                 enr,ienre,itestm)
...
820           format(15x,'accuracy in decimal digits = ',i2,/,10x,
     1               'r2 = ',f19.15,i5,5x,'r2d = ',f19.15,i5)
825           format(15x,'accuracy in decimal digits = ',i2,/,10x,
     1               'r2 = ',f34.30,i5,5x,'r2d = ',f34.30,i5)
...
900           continue
     ...
     1                  enrneg,drhor,dneg,idneg,nsdneg,nsdrhor1)
              kflagl=0
     ...
     2                   drhor,nsdrhor1,dc01,idc01,dneg,idneg,nsdneg,
     3                   dfnorm,idfe,dmfnorm,idmfe,prx,pdrx,qdrat,qdqr,
     4                   qdm1,iqdm1,qdl,iqdl,qrat,qm1,iqm1,ql,iql,fajo,
     5                   ifajo,ifsub,jsub,termpq,itermpq,ioppsum,
     6                   iopqnsum,r1cin,ir1ein,r1dcin,ir1dein,naccr1,
     7                   itestm,r2lc,ir2le,r2dlc,ir2dle,jleg,jlegp,
     8                   jflagl,naccleg,kflagl,isub,isubd)
              iopleg=2
              nacclegs=naccleg
...
1050          continue
     ...
     1             .and.ndec-jsub.lt.minacc+1.and.x.gt.0.05e0_knd)
     2             iopeta=3
...
1210          continue
     ...
     5                   ir2ee,r2dec,ir2dee,nacceta,jeta,naccd)
              ...
              ir2de(li)=ir2dee
1260          continue
1270          if(knd.eq.kindd) write(40,1280) naccetas,etaval,nee,r2ec,
     1                                    ir2ee,r2dec,ir2dee
              if(knd.eq.kindq) write(40,1285) naccetas,etaval,nee,r2ec,
     1                                    ir2ee,r2dec,ir2dee
1280          format(15x,'r2eta accuracy = ',i2,' decimal digits; eta',
     1               ' = ',f12.9,'; nee = ',i4,/,10x,'r2 = ', f19.15,i5,
     2               5x,'r2d = ',f19.15,i5)
1285          format(15x,'r2eta accuracy = ',i2,' decimal digits; eta',
     1               ' = ',f12.9,'; nee = ',i4,/,10x,'r2 = ', f34.30,i5,
     2               5x,'r2d = ',f34.30,i5)
              iopeta=3
...
1350            format(1x,i5,2x,4(f17.14,i6,2x),i2,'e')
                go to 1400
                end if
...
1380          format(1x,i5,2x,4(f17.14,i6,2x),i2,'w')
              go to 1400
1390          continue
                if(ix.eq.1) then
                r2c(li-1)=r1c(li)
...
1405          continue
     ...
     1             ' est. acc. = ',naccrp, ' digits for x = ',x,' c = ',
     2             c,' m = ',m,' l = ',l-1
                if(ix.eq.1.and.naccr.lt.8) write(60,*)
     1             ' est. acc. = ',naccr, ' digits for x = ',x,' c = ',
     2             c,' m = ',m,' l = ',l
                end if
              if(ioprad.eq.2.and.li.ge.matlilim.and.naccr.lt.8)
     1            write(60,*) ' est. acc. = ',naccr, ' digits for x = ',
     2            x,' c = ', c,' m = ',m,' l = ',l
              if(ioprad.eq.1.and.naccr1.lt.8) write(60,*)
     1           'est. r1 acc. = ',naccr1,' digits for x = ',x,' c = ',
     2           c,' m = ',m,' l = ',l
              naccrp=naccr
...
1410          if(l.eq.m) lims1=3*ndec+int(c)
     ...
     1                   maxt,maxd,maxp,enr,dmlms,idmlmse,pr,pdr,
     2                   pdnorm,ipdnorm,pnorm,ipnorm,pdtempe,ipdtempe,
     3                   pdtempo,ipdtempo,ptempe,iptempe,ptempo,iptempo,
     4                   itestm,kindd,kindq,s1c,is1e,s1dc,is1de,naccs,
     5                   naccds,jang,dmlms1,idmlms1e)
                do 1500 jarg=1,narg
                if(ioparg.eq.0.and.iopang.eq.1) write(50,1420)
     1                arg(jarg),naccs(jarg)
                if(ioparg.eq.1.and.iopang.eq.1) write(50,1430)
     1                barg(jarg),naccs(jarg)
                if(ioparg.eq.0.and.iopang.eq.2) write(50,1425)
     1                arg(jarg),naccs(jarg),naccds(jarg)
                if(ioparg.eq.1.and.iopang.eq.2) write(50,1435)
     1                barg(jarg),naccs(jarg),naccds(jarg)
1420            format(1x,'theta = ',e24.15,'   accuracy = ',i2,
     1                 ' digits.')
1430            format(1x,'eta = ',e24.15,'   accuracy = ',i2,
     1                 ' digits.')
1425            format(1x,'theta = ',e24.15,'   s1 and s1d accuracy = ',
     1                 i2,' and ',i2,' digits.')
1435            format(1x,'eta = ',e24.15,'   s1 and s1d accuracy = ',
     1                 i2,' and ',i2,' digits.')
                if(ioparg.eq.0.and.iopang.eq.1) write(30,1440)
     1                arg(jarg),s1c(jarg),is1e(jarg),naccs(jarg)
                if(ioparg.eq.0.and.iopang.eq.2) write(30,1450)
     1                arg(jarg),s1c(jarg),is1e(jarg),s1dc(jarg),
     2                is1de(jarg),naccs(jarg),naccds(jarg)
                if(ioparg.eq.1.and.iopang.eq.1) write(30,1460)
     1                barg(jarg),s1c(jarg),is1e(jarg),naccs(jarg)
                if(ioparg.eq.1.and.iopang.eq.2) write(30,1470)
...
1450            format(1x,f17.14,2x,f17.14,2x,i5,2x,f17.14,2x,i5,2x,i2,
     1                    ', ',i2)
1460            format(1x,f17.14,2x,f17.14,2x,i5,2x,', ',i2)
1470            format(1x,f17.14,2x,f17.14,2x,i5,2x,f17.14,2x,i5,2x,i2,
     1                    ', ',i2)
...
c
     ...
     4                    iptempo,itestm,kindd,kindq,s1c,is1e,s1dc,
     5                    is1de,naccs,naccds,jang,dmlms1,idmlms1e)
c
...
c               dmlms   : characteristic of the value of the d
c                         coefficient with index l - m using the
c                         Meixner-Schafke normalization
...
c               itestm  : number of matching digits for the forward
c                         and backward recursion for d coefficient
c                         ratios
...
c  real(knd) scalars and arrays
        real(knd) adec,aj,c,dcon,dec,dmlms,dmlms1,dnew,dnewd,
     1            dold,doldd,factor,fterm,rm2,rm2m1,rm2m3,rm2p1,
...
c  real(knd) scalars and arrays
     ...
     1            r1c,r1ca,r1dc,r1dcoef,r1temp,r1tempa,r1dtemp,
     2            r1dtempa,r1top,r1topa,r1dtop,spos,sposa,sdpos,ten,
     3            term,teste,testeo,x,x2
...
c  real(knd) scalars and arrays
     ...
     2            r1dc,r1dcoef1,r1dcoef2,r1dtemp,r1temp,sumdnp,
...
c  real(knd) scalars and arrays
     ...
     1            dnewd,dold,doldd,dc01,ri,rm,rm2,r2c,r2dc,r2dpos,rs,
     2            r2dtemp,r2pos,r2temp,ten,teste,testeo,x
...
c
     ...
     1                    nex,maxd,maxmp,maxpdr,maxdr,maxq,enr,enrneg,
     2                    drhor,nsdrhor1,dc01,idc01,dneg,idneg,nsdneg,
     3                    dfnorm,idfe,dmfnorm,idmfe,prx,pdrx,qdr,qdqr,
     4                    qdm1,iqdm1,qdl,iqdl,qr,qm1,iqm1,ql,iql,fajo,
     5                    ifajo,ifsub,jsub,termpq,itermpq,ioppsum,
     6                    iopqnsum,r1c,ir1e,r1dc,ir1de,naccr1,itestm,
     7                    r2c,ir2e,r2dc,ir2de,jleg,jlegp,jflagl,naccleg,
     8                    kflagl,nsubleg,nsubdleg)
c
...
c                         the value of the d coefficient d(n = l - m) using
...
c               jsub    : larger of the subtraction errors in the
...
c  real(knd) scalars and arrays
     ...
     3            qsum,qsump,r1c,r1dc,r2c,r2dc,rm,spsum,spsump,spdsum,
     4            spdsump,ten,termpq,test,testd,testm,testdm,testp,tm,
     5            wronc,wronca,wroncb,wront,x,xden,xdrhor,xrhs
...
190     format(8x,'r2leg: qsum, psum and qnsum series converged in ',i6,
     ...
     2        ' terms avail.',/,14x,i2,' and ',i2,' digits of sub.',
...
200     format(8x,'r2leg: qsum and psum series converged in ',i6,
     ...
     2        14x,i2,' and ',i2,' digits of sub. error in r2 and r2d;',
...
c  real(knd) scalars
     ...
     1            r2d,r2dc,r3,sumi,sumdi,sumr,sumdr,ten,term,termi,
     2            termdi,termr,termdr,x,xx
c  real(knd) vectors
...
110     format(8x,'r2neu0 (eta=0) : numerator converged in ',i6,
     1         ' terms; ',i6,' terms available.',/,15x,'best r2 at ',i6,
     2         ' terms with convergence to ',i2,' digits;',i3,' digits',
     3         ' sub. error.',/,15x,'best r2d at ',i6,' terms with',
     4         ' convergence to ',i2,' digits; ',i2,' digits sub. '
     5         'error')
...
c
     ...
     1                     kindd,kindq,eigval,enr,ienr,itestm)
c
...
c               enr   : array of scaled ratios of successive d coefficients
...
140     continue
     ...
     1           eigval,eigstart
150     format(1x,'l =',i5,6x,'eigenvalue =',e24.15,
     1               '; estimate =',e24.15)
155     format(1x,'l =',i5,6x,'eigenvalue =',e39.31,
     1               '; estimate =',e39.31)
...
240     format(15x,i6,' d coefs. Forward and backward recursion ',
     1         'values of d(n+2)/d(n) match to ',i2,' digits at n = ',
     2         i6)
          do 250 n=1,nlim
          enr(n)=enrf(n)
250       continue
...
70      format(17x,'Morse-Feshbach norm. converged'
     1         ' in ',i6,' terms with ',i2,' digit subtr. error.')
...
120     format(17x,'Flammer norm. converged in ',
     1         i6,' terms with ',i2,' digit subtr. error.')
c
...
c                        ratios minus 2m calculated for given m,lnum,
c                        c,ndec,and x
...
c  real(knd) scalars and arrays
     ...
     1            qupp,qupp0,qupp1,q00,q0m,q1m,q11,qmmm1,rin,rin1,
