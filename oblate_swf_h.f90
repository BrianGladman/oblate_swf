...
!  expansion d coefficients and eigenvalues is given in the readme file.
    ...
    real(knd) aj1, aj2, apcoef, apcoefn, api, c2, c4, coefn, coefme, &
         ...
         rm2, r1cin, r1cm, r1dcin, r1c_, r1dcm, r1dc_, r1ec, r1dec, &
         r2c_, r2dc_, r2lc, r2dlc, r2l1c, r2dl1c, r2nc, r2dnc, r2ec, &
         r2dec, step1, step2, step3, ten, termpq, t1, t2, t3, t4, t5, t6, &
         t7, wm, xb, xbninp, xl, xneu, wronc, wronca, wroncb, wront, &
         ang, apcoef1, etaval1, pcoefe, pcoefet, pcoefo, pdcoefe, &
         pdcoefo, pdcoefet, pcoefe1, pcoefet1, pcoefo1, pdcoefe1, &
         pdcoefo1, pdcoefet1

    integer, dimension(:), allocatable :: &
        match, iqdl, iql, ifajo, ibese, ineue2, ipnormint, ineuee, ibesee, &
        ipdnorm, ipdnorma, ipnorm, ipnorma, ipdtempe, ipdtempo, iptempe, &
        iptempo, is1e_, is1de_, naccs_, naccds_, neeb, neeb1, limpsv, &
        limp1sv, limnsv, jelimsv, jelim1sv, limjsv

    integer, dimension(:,:), allocatable :: ibesesv, ineuesv
    
    real(knd), dimension(:), allocatable :: &
        eig, qdl, ql, fajo, enr, bliste, gliste, blisto, glisto, drhor, pint1, pint2, &
        pint3, pint4, rpint1, rpint2, sbesf, sbesdf, sbesfe, sbesdfe, pnormint, sbesdr, &
        sbesn, sneun2, sneudr2, sneune, sneudre, enrneg, qdqr, sneuf2, sneudf2, prat1, &
        sneufe, sneudfe, alpha, beta, coefa, coefb, coefc, coefd, coefe, gamma, pdratt, &
        pratb, pratt, pratb1, pratt1, pdratt1, prx, pdrx, qrat, qdrat, qrat1, qdrat1, etainp, &
        pdnorm, pdnorma, pnorm, pnorma, pdtempe, pdtempo, ptempe, ptempo, s1c_, s1dc_, xin, &
        xlninp, wr, xr, eigt, f, d, e, eta, xbn, xln, wmeta2, sbesne, sbesdre
                        
    real(knd), dimension(:,:), allocatable :: &
        sbesfsv, sbesdfsv,sneudrsv, sbesnsv, sbesdrsv, sneunsv, sneufsv, sneudfsv, pdr, pdrat, &
        pr, prat, pratbsv, prattsv, pdrattsv, pratbsv1, prattsv1, pdrattsv1
...
10   maxp = max(maxn, maxp, maxpdr)
    ...
    allocate (match(lnum), iqdl(lnum), iql(lnum), ifajo(lnum + 1), ibese(maxlp), &
              ineue2(maxlp), ipnormint(maxlp), ineuee(maxlp), ibesee(maxlp), &
              ipdnorm(maxt), ipdnorma(maxt), ipnorm(maxt), ipnorma(maxt), &
              ipdtempe(maxt), ipdtempo(maxt), iptempe(maxt), iptempo(maxt), &
              is1e_(maxt), is1de_(maxt), naccs_(maxt), naccds_(maxt), neeb(jnenmax), &
              neeb1(jnebmax), limpsv(jnenmax), limp1sv(jnebmax), limnsv(jnenmax), &
              jelimsv(jnenmax), jelim1sv(jnebmax), limjsv(jnebmax))
            
    allocate (ibesesv(jnebmax, maxlp), ineuesv(jnenmax, maxlp))

    allocate (sbesfsv(jnebmax, maxj), sbesdfsv(jnebmax, maxj),sneudrsv(jnenmax, maxlp), &
              sbesnsv(jnebmax, maxlp), sbesdrsv(jnebmax, maxlp), sneunsv(jnenmax, maxlp), &
              sneufsv(jnenmax, maxn), sneudfsv(jnenmax, maxn), pdr(maxt, maxp), &
              pdrat(maxt, maxp), pr(maxt, maxp), prat(maxt, maxp), pratbsv(jnenmax, maxp), &
              prattsv(jnenmax, maxp), pdrattsv(jnenmax, maxp), pratbsv1(jnebmax, maxp), &
              prattsv1(jnebmax, maxp), pdrattsv1(jnebmax, maxp))

    allocate (eig(lnum), qdl(lnum), ql(lnum), fajo(lnum + 1), enr(maxd), bliste(maxd), &
              gliste(maxd), blisto(maxd), glisto(maxd), drhor(maxdr), pint1(maxint), &
              pint2(maxint), pint3(maxint), pint4(maxint), rpint1(maxint), rpint2(maxint), &
              sbesf(maxj), sbesdf(maxj), sbesfe(maxj), sbesdfe(maxj), pnormint(maxlp), &
              sbesdr(maxlp), sbesn(maxlp), sneun2(maxlp), sneudr2(maxlp), sneune(maxlp), &
              sneudre(maxlp), enrneg(maxmp), qdqr(maxmp), sneuf2(maxn), sneudf2(maxn), &
              prat1(maxn), sneufe(maxn), sneudfe(maxn), alpha(maxp), beta(maxp), coefa(maxp), &
              coefb(maxp), coefc(maxp), coefd(maxp), coefe(maxp), gamma(maxp), pdratt(maxp), &
              pratb(maxp), pratt(maxp), pratb1(maxp), pratt1(maxp), pdratt1(maxp), prx(maxpdr), &
              pdrx(maxpdr), qrat(maxq), qdrat(maxq), qrat1(maxq), qdrat1(maxq), etainp(maxt), &
              pdnorm(maxt), pdnorma(maxt), pnorm(maxt), pnorma(maxt), pdtempe(maxt), &
              pdtempo(maxt), ptempe(maxt), ptempo(maxt), s1c_(maxt), s1dc_(maxt), xin(maxt), &
              xlninp(maxt), wr(ngau), xr(ngau), eigt(maxe2), f(maxe2), d(maxe), e(maxe), &
              eta(neta), xbn(neta), xln(neta), wmeta2(neta), sbesne(maxlp), sbesdre(maxlp))
...
!  determine oblate radial functions of both kinds when x = 0
       ...
       if(m == 0) idneg = 0
       nsdneg = 0
       naccr_ = min(ndec - max(jsub, ksub, nsdneg) - 1, itestm - 1, ndec - 2)
       if(naccr_ < 0) naccr_ = 0
...
390      r1dc(li) = 0.0e0_knd
       ...
       r2dc(li) = r2dc(li) * ten
       ir2de(li) = ir2de(li) - 1
400      if(naccr_ == 0) r2c(li) = 0.0e0_knd
       if(naccr_ == 0) ir2e(li) = 0
       if(li /= 1) fac2 = -real(l + m - 1, knd) * (l - m - 1) * fac2/ &
                (real(l - m, knd) * (l + m))
       if(naccr_ == 0) go to 450
...
430      if(ioprad == 1) go to 450
       ...
       r2c(li) = r2c(li) * ten
       ir2e(li) = ir2e(li) - 1
440      if(naccr_ == 0) r2dc(li) = 0.0e0_knd
       if(naccr_ == 0) ir2de(li) = 0
...
560      format(4x,'r1 and r1d calculation')
       ...
       call r1bes(l, m, c, x, limr1, ndec, maxd, enr, maxj, maxn, maxlp, &
             ...
             prat1, pcoefn, ipcoefn, dmfnorm, idmfe, ir1ep, r1c_, &
             ir1e_, r1dc_, ir1de_, jbes1, nsub, nsubd)
       jbes = jbes1
       iopbes = 2
if (debug) then
       if(knd == kindd) write(40, 570) r1c_, ir1e_, r1dc_, ir1de_
       if(knd == kindq) write(40, 575) r1c_, ir1e_, r1dc_, ir1de_
...
800      continue
       ...
       call r2int(l, m, c, x, limint, ndec, nex, maxd, enr, dc01, idc01, &
             ...
             ipnormint, coefme, coefmo, ipint, r2c_, ir2e_, &
             r2dc_, ir2de_, jint, coefn, icoefn, isub, isubd)
       if(ipint == 0) ipint = 1
       naccrs = naccr_
       wronca = r1c(li) * r2dc_ * ten ** (ir1e(li) + ir2de_)
       wroncb = r2c_ * r1dc(li) * ten ** (ir2e_ + ir1de(li))
       wronc = wronca - wroncb
...
if (debug) then
       if(knd == kindd) write(40, 820) naccint, r2c_, ir2e_, r2dc_, ir2de_
       if(knd == kindq) write(40, 825) naccint, r2c_, ir2e_, r2dc_, ir2de_
...
!     r2 calculation using a Legendre expansion and joining factor
       if(iopleg == 0 .or. ndec - jsub < naccr_ - 2) go to 940
...
900      continue
       ...
       if(naccleg < naccr_ .or. (naccleg == naccr_ .and. naccint == &
          naccr_ .and. jflagl == 1)) go to 910
       naccr_ = naccleg
       r2c(li) = r2lc
       ir2e(li) = ir2le
       r2dc(li) = r2dlc
...
910      continue
       if(naccleg > minacc + 1 .and. nacclegp > minacc + 1) &
        ...
        if(m <= 40 .and. c <= 60.0e0_knd .and. c >= 10.0e0_knd .and. &
          li < nbp .and. iopleg1 == 0 .and. naccr_ < minacc .and. &
...
!     equal to 0
       if(iopneu0 == 0 .or. ndec - jsub < naccr_) go to 1080
...
1310     continue
       if(naccr_ >= minacc .or. (iopeta /= 0 .and. naccetas > &
          minacc)) go to 1320
...
1320     continue
       ...
       if(iopint /= 0 .and. naccint > nacintsa) nacintsa = naccint
        if(l == m .and. iopint /= 0 .and. naccint == naccr_) then
        if(iopeta /= 0) iflageta = 1
        iopeta = 0
        end if
       if(naccr_ > 0) go to 1330
       naccr_ = 0
       r2c(li) = 1.0e0_knd
       ir2e(li) = -ndec
       r2dc(li) = 1.0e0_knd
...
if (output) then
        write(20, 1380) l, r1c(li), ir1e(li), r1dc(li), ir1de(li), &
                         r2c(li), ir2e(li), r2dc(li), ir2de(li), -naccr_, chr_e
end if
        go to 1400
        end if
...
if (debug) then
        ...
        naccr_ = -int(log10(abs((wronc - wront) / wront) + dec))
        if(naccr_ < 0) naccr_ = 0
        naccr_ = min(naccr_, naccr1, itestm - 1)
if (output) then
        write(20, 1380) l, r1c(li), ir1e(li), r1dc(li), ir1de(li), &
...
1370     format(8x,'Values for r2 and r2d for ','l = ',i5, &
           ' are given by -r1 and -r1d for l = ',i5)
       naccr_ = min(naccr_, naccr1, itestm - 1)
        if(jflagl == 1 .and. naccr_ == naccleg .and. &
          naccr_ /= naccint) then
if (output) then
        write(20, 1380) l, r1c(li), ir1e(li), r1dc(li), ir1de(li), &
                         r2c(li), ir2e(li), r2dc(li), ir2de(li), -naccr_, chr_e
...
if (output) then
        write(20, 1380) l, r1c(li), ir1e(li), r1dc(li), ir1de(li), &
             r2c(li), ir2e(li), r2dc(li), ir2de(li), naccr_, chr_w
end if
        end if
...
if (warn) then
        ...
        if(ix == 1 .and. naccrp < 8) write(60,*) &
          ...
          c,' m = ',m,' l = ',l - 1
        if(ix == 1 .and. naccr_ < 8) write(60,*) &
          ' est. acc. = ',naccr_, ' digits for x = ',x,' c = ', &
          c,' m = ',m,' l = ',l
        end if
       if(ioprad == 2 .and. li >= matlilim .and. naccr_ < 8) &
       ...
       if(ioprad == 2 .and. li < matlilim .and. ix == 1) naccr(li - 1) = naccrp
       if(ioprad == 2 .and. li < matlilim .and. ix == 1) naccr(li) = naccr_
       if(ioprad == 2 .and. li >= matlilim) naccr(li) = naccr_
       if(ioprad == 1) naccr(li) = naccr1
       naccrp = naccr_
...
1410     if(l == m) lims1 = 3 * ndec + int(c)
       ...
       call s1leg(l, m, c, iopang, iopnorm, arg, narg, lims1, ndec, nex, &
...
if (debug) then
        ...
        if(iopang == 2) write(50, 1435) arg(jarg), naccs_(jarg), naccds_(jarg)
1430      format(1x,'eta = ',f17.14,'   accuracy = ',i2,' digits.')
1435      format(1x,'eta = ',f17.14,'   s1 and s1d accuracy = ',i2,' and ',i2,' digits.')
end if
if (output) then
        if(iopang == 1) write(30, 1460) arg(jarg), s1c_(jarg), is1e_(jarg), naccs_(jarg)
        if(iopang == 2) write(30, 1470) arg(jarg), s1c_(jarg), is1e_(jarg), s1dc_(jarg), &
                                is1de_(jarg), naccs_(jarg), naccds_(jarg)
1460      format(1x, f17.14, 2x, f17.14, 2x, i5, 2x,', ',i2)
1470      format(1x, f17.14, 2x, f17.14, 2x, i5, 2x, f17.14, 2x, i5, 2x, i2,', ',i2)
end if
if (debug) then
        if(knd == kindd .and. iopang == 1) write(50, 1480) s1c_(jarg), is1e_(jarg)
        if(knd == kindd .and. iopang == 2) write(50, 1490) s1c_(jarg), is1e_(jarg), s1dc_(jarg), is1de_(jarg)
        if(knd == kindq .and. iopang == 1) write(50, 1485) s1c_(jarg), is1e_(jarg)
        if(knd == kindq .and. iopang == 2) write(50, 1495) s1c_(jarg), is1e_(jarg), s1dc_(jarg), is1de_(jarg)
end if
...
1540    continue
    ...
    deallocate (sbesfsv, sbesdfsv,sneudrsv, sbesnsv, sbesdrsv, sneunsv, sneufsv, sneudfsv, pdr, &
                pdrat, pr, prat, pratbsv, prattsv, pdrattsv, pratbsv1, prattsv1, pdrattsv1)
    deallocate (eig, qdl, ql, fajo, enr, bliste, gliste, blisto, glisto, drhor, pint1, pint2, &
                pint3, pint4, rpint1, rpint2, sbesf, sbesdf, sbesfe, sbesdfe, pnormint, sbesdr, &
                sbesn, sneun2, sneudr2, sneune, sneudre, enrneg, qdqr, sneuf2, sneudf2, prat1, &
                sneufe, sneudfe, alpha, beta, coefa, coefb, coefc, coefd, coefe, gamma, pdratt, &
                pratb, pratt, pratb1, pratt1, pdratt1, prx, pdrx, qrat, qdrat, qrat1, qdrat1, etainp, &
                pdnorm, pdnorma, pnorm, pnorma, pdtempe, pdtempo, ptempe, ptempo, s1c_, s1dc_, xin, &
                xlninp, wr, xr, eigt, f, d, e, eta, xbn, xln, wmeta2, sbesne, sbesdre)
    deallocate (ibesesv, ineuesv)
    deallocate (match, iqdl, iql, ifajo, ibese, ineue2, ipnormint, ineuee, ibesee, &
                ipdnorm, ipdnorma, ipnorm, ipnorma, ipdtempe, ipdtempo, iptempe, &
                iptempo, is1e_, is1de_, naccs_, naccds_, neeb, neeb1, limpsv, &
                limp1sv, limnsv, jelimsv, jelim1sv, limjsv)
    close(60)
